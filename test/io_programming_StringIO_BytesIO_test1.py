...
# -*- coding: utf-8 -*-
'''
    # IO编程-StringIO和BytesIO：StringIO和BytesIO综合练习
    # 使用场景：
        StringIO和BytesIO综合练习
        stringIO 只能操作str，如果要操作二进制数据，就需要用到BytesIO
        关于StringIO和BytesIO的seek()和tell()两个方法的使用
            （1）、tell 方法获取当前文件读取指针的位置；
            （2）、seek 方法，用于移动文件读写指针到指定位置,有两个参数，第一个offset: 偏移量，需要向前或向后的字节数，正为向后，负为向前；第二个whence: 可选值，默认为0，表示文件开头，1表示相对于当前的位置，2表示文件末尾；
            特别注意：用seek方法时，需注意，如果你打开的文件没有用'b'的方式打开，则offset无法使用负值哦   
...
'''
    #注：Python中doc中关于seek()和tell()方法的注释：
****************************************************************************
    def seek(pos, whence)
