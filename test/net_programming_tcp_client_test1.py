...
# 因此，我们连接新浪服务器的代码如下：s.connect(('www.sina.com.cn', 80))；注意参数是一个tuple，包含地址和端口号。
...
print('http相应头部信息为：', header.decode('utf-8'))
# 把接收的数据写入文件
with open('net_programming_tcp_client_test1_sina.html', 'wb') as f:
    f.write(html)

# 现在，只需要在浏览器中打开这个*_sina.html文件，就可以看到新浪的首页了。
