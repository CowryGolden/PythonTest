...
with open('dump_json_str.txt', 'w', encoding='utf-8',errors='ignore') as f:
    json.dump(d, f)
print('将Python的dict类型转换为JSON字符串并写入dump_json_str.txt文件成功！')

# 要把JSON反序列化为Python对象，用loads()或者对应的load()方法，前者把JSON的字符串反序列化，后者从file-like Object中读取字符串并反序列化：
# d = json.loads(json_str)  # dict对象；isinstance(d, dict) : True
with open('dump_json_str.txt', 'r', encoding='utf-8',errors='ignore') as f:
    d = json.load(f)
print('从文件dump_json_str.txt中读取JSON字符串并将其转换为Python的dict类型为 :', d)
